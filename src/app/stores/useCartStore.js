  ...
  return {
          ...
          const syncWithAPI = async () => {
            try {
              ...
              if (response.success && response.data?.items && Array.isArray(response.data.items) && response.data.items.length > 0) {
                ...
                // If the API returns success but no items/empty list, and we have local items,
                // do NOT clear the local items. This might be a transient state or sync delay.
                if (loadedItems.length > 0) {
                  console.debug('[Cart] API returned empty cart while local has items, keeping local');
                  return;
                }
              }
            } catch (err) {
    ...
    removeFromCart: async (productId) => {
      ...
      try {
        ...
        // Remove from local cart - strictly match by ID or productId
        const newItems = cartItems.filter(
          item => {
            const itemId = String(item.id || item.productId);
            return itemId !== pid && String(item.productId) !== pid;
          }
        );
        const newTotals = calculateTotals(newItems);
    ...
    updateQuantity: async (productId, quantity) => {
      ...
      const updatedItems = cartItems.map(item =>
        (String(item.id) === pid || String(item.productId) === pid)
      ...
      // 2. BACKGROUND SYNC: Update server and check stock
      try {
        ...
        // Optional: Background stock check (could be debounced or skipped for simple updates)
        // For now, we've already updated the UI, so we just return success
        return { success: true };
      } catch (err) {
        console.error('[Cart] Quantity sync failed:', err);
        // We don't necessarily revert here to keep the UI "lively", 
        // but we could mark the item as "syncing/error" if we had that state.
        return { success: true }; // Still return true for perceived success
      }
    },
